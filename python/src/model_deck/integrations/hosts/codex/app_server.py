"""B10 Codex app-server bridge: the desktop JSON-RPC mapping extracted behind a host adapter.

This module owns the seam between Model Deck and a Codex desktop process
speaking the JSON-lines app-server protocol. It is intentionally
self-contained:

* Only the Python standard library is imported at module scope.
* A narrow structural protocol/alias surface describes what the host needs
  from the catalog, router, instruction builder, and process launcher. The
  package code never reaches into ``LocalRouter``, ``RoutingRegistry``,
  pricing, provider adapters, or any root-private storage.
* Thread ownership, per-thread locking, config overlay, exact-turn
  cancellation only after backend success, approval/tool forwarding,
  per-process ``-c``/MCP overrides, and host-bound ``gpt-*`` subscription
  routing are all preserved.

The root ``provider_bridge.py`` becomes a thin legacy wrapper that re-exports
the public surface and supplies root-owned defaults (model roster
composition and the root ``discover_runtime``).
"""
...
# ---------------------------------------------------------------------------
# Public constants and helpers — preserved from the original root mapping.
# ---------------------------------------------------------------------------
...
def routing_instructions(registry, price_table=None, benchmark_lines=None):
    """The structural routing rules plus the MCP tool guidance.

    The roster of added models with cached prices and benchmark evidence lives
    in the root ``provider_bridge.py`` (via ``spawn_benchmarks.model_choice_lines``)
    and is composed there as the ``instruction_builder`` callable. The default
    ``instruction_builder`` for this package only concatenates the
    text-only routing/MCP sections so the package stays independent of the root.
    """
...
def _resolve_mcp_script(explicit):
    if explicit is not None:
        candidate = Path(explicit)
        return candidate if candidate.is_file() else None
    base = Path(__file__).resolve().parent
    # Walk upward looking for the shared MCP server script. The package is
    # reachable from either the source tree or a deployed bundle that keeps
    # ``model_deck_mcp.py`` beside ``app_server.py``.
    seen = set()
    for ancestor in [base, *base.parents]:
        if ancestor in seen:
            continue
        seen.add(ancestor)
        candidate = ancestor / 'model_deck_mcp.py'
...
def mcp_server_arguments(script=None, python=None):
    ...
    resolved = _resolve_mcp_script(script)
    if resolved is None:
        return []
    table = {
...
# ---------------------------------------------------------------------------
# Narrow structural dependencies — the package never imports the concrete
# classes that live outside this module.
# ---------------------------------------------------------------------------


@runtime_checkable
class CatalogProjection(Protocol):
    """Structural projection of the registered-model registry the bridge needs."""
...
class RouterProcess(Protocol):
    """Structural projection of the running router/process companion."""

    def codex_arguments(self) -> list: ...
...
InstructionBuilder = Callable[..., str]
ProcessLauncher = Callable[..., dict]
...
class AppServerBridge:
    """The extracted JSON-lines app-server mapping.

    The constructor receives only narrow structural dependencies so the host
    adapter can be exercised without importing the root composition graph.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.registry = catalog  # preserved name for root compatibility
        ...
        self.instruction_builder = (
            instruction_builder if instruction_builder is not None else routing_instructions
        )
        self.process_launcher = (
            process_launcher if process_launcher is not None else _default_process_launcher
        )
    ...
    async def dispatch(self, method, params):
        ...
        if method == 'thread/start':
            ...
            params['developerInstructions'] = '\n\n'.join(filter(None, [instructions,
                self.instruction_builder(self.registry, price_table, benchmark_lines)]))
    ...
    async def run(self, argv):
        ...
        if self.router is None:
            ...
            raise BridgeError('AppServerBridge.run requires a router. Compose one in the root entrypoint.')
        self.router.start()
        try:
...
# ---------------------------------------------------------------------------
# Lazy default process launcher — keeps the package import graph stdlib-only.
# ---------------------------------------------------------------------------


def _default_process_launcher():
    """Lazy default: defer the runtime import until the launcher is called.

    The package's ``runtime`` module performs the same desktop discovery the
    root's ``codex_runtime.discover_runtime`` does, so the bridge stays
    importable in environments where the runtime module is unavailable.
    """
    from . import runtime as _runtime  # noqa: WPS433  intentional lazy import
    descriptor = _runtime.discover_runtime()
    return {
        'application_path': str(descriptor.application_path),
        'executable_path': str(descriptor.executable_path),
        'bundle_identifier': descriptor.bundle_identifier,
    }
