...
def validate_isolated_roots(
    state_root: Union[Path, str],
    artifact_root: Union[Path, str],
    socket_root: Optional[Union[Path, str]] = None,
    *,
    source_root: Path,
    ...
    if not state.is_absolute() or not artifact.is_absolute():
        raise IsolatedRootGuardError("state_root and artifact_root must be absolute paths")
    ...
    if state_real == artifact_real:
        raise IsolatedRootGuardError("state_root and artifact_root must not be the same path")
    ...
    socket_real: Optional[Path] = None
    if socket_root is not None:
        socket = Path(socket_root)
        if not socket.is_absolute():
            raise IsolatedRootGuardError("socket_root must be an absolute path when provided")
        ...
        if socket_real in (state_real, artifact_real) or _one_contains_other(state_real, socket_real) or _one_contains_other(artifact_real, socket_real):
            raise IsolatedRootGuardError("socket_root must be disjoint from state_root and artifact_root")
...
def _require_directory_root(path: Path, *, label: str, source_root: Path) -> Path:
    if path.is_symlink():
        raise IsolatedRootGuardError(f"{label} must not be a symlink alias: {path}")
    reject_symlink_parent_aliases(path, label=label)
    if path.exists() and not path.is_dir():
        raise IsolatedRootGuardError(f"{label} must be a directory, not an existing file: {path}")
    resolved = reject_protected_path(path, label=label, source_root=source_root)
    if resolved.is_symlink():
        raise IsolatedRootGuardError(f"{label} resolves through a symlink alias: {path}")
    return resolved
