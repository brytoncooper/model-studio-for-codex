...
def protected_roots(source_root: Path | None = None) -> tuple[Path, ...]:
    if source_root is None:
        raise DevelopmentGuardError("protected_roots requires source_root from repo_root_from_script")
...
def canonicalize_path(path: Path) -> Path:
    try:
        ...
        raise DevelopmentGuardError(f"Cannot resolve path: {path}") from exc
...
def reject_source_tree_usage(path: Path, *, label: str, source_root: Path) -> None:
    ...
    containing_source: Path | None = None
