...
| B10 | Codex host adapter and compatibility profile | **Complete** | [Codex host guide](../../../python/src/model_deck/integrations/hosts/codex/README.md), extracted app-server/runtime adapters, frozen engine host operations, thin legacy wrapper, and D1a reuse behind V2's attachment boundary | None for applicable B10 acceptance. The [updated isolated qualification](CODEX-DESKTOP-QUALIFICATION-2026-09-14.md) proves the real installed app-server workflow; installation/live cutover remain B27. | None |
...
| B16 | Usage, pricing, benchmark sources and refresh | **Implementation remaining** | [usage owner](../../../python/src/model_deck/engine/usage/README.md), public `engine.v1.usage.query`, V2 usage UI/live token evidence; B16 delivery 2026-09-15 (commits 09336e4-70328ce) added [evidence source adapters](../../../python/src/model_deck/adapters/evidence/README.md), [engine evidence cache](../../../python/src/model_deck/engine/evidence/README.md) with age-bearing provenance and atomic refresh-or-keep-last-good behavior, [first-party refresh jobs](../../../python/src/model_deck/engine/jobs/README.md) backing `engine.v1.prices.refresh`/`engine.v1.benchmarks.refresh`, the `engine.v1.prices.query`/`engine.v1.benchmarks.query`/`engine.v1.usage.summary` operations, MCP evidence reads with legacy fallback and a V2 price-snapshot row, and settled/estimated/subscription `cost_kind` projection in both the engine (`project_cost_kind`) and the legacy Swift usage ledger; new coverage in `test_price_source.py`, `test_benchmark_source.py`, `test_evidence_source_bridge.py`, `test_evidence_cache.py`, `test_sqlite_evidence.py`, `test_evidence_refresh_jobs.py`, `test_evidence_refresh_job_service.py`, `test_first_party_job_runner.py`, `test_jobs_use_cases.py`, `test_usage_records.py`, `test_usage_reconciliation.py`, `test_usage_bootstrap.py`, `test_mcp_evidence_reads.py`, and Swift `EngineUsageServiceTests`/`UsageRecordCostKindTests`/`EvidencePresenterTests`/`V2WorkspacePriceSnapshotTests`; full gate green (development-guard 37, contracts, engine 1629, swift 243, migration 321, providers 335, extensions 362, 0 failures) | No subscription-allowance source is composed, so `subscription_allowance` never appears in the engine's `usage.summary` (`SubscriptionAllowancePort` has no adapter); benchmark `provider_model_id` is never mapped from a feed's `source_model_ref` to a registered model; only one benchmark feed is composed per refresh, with no cross-feed merge/rank; `EvidenceResourceExhaustedError` still maps to a flat `internal` rather than `resource_exhausted` in dispatch. Provider monetary cost remains unknown. | B19 public job execution |
| B17 | Kernel registration and built-in composition | **Integration remaining** | [kernel](../../../docs/kernel.md), [composition](../../../python/src/model_deck/engine/kernel_composition.py), authenticated generic invocation tests; the former engine-to-plugin-runtime edge is removed behind the application gateway; B17 delivery 2026-09-15 (commits 09336e4, 1198ab3) added [engine.builtins](../../../python/src/model_deck/engine/builtins/README.md) — a capability vocabulary, 12 `FeatureDescriptor`s covering every `_OPERATION_CATALOG` operation plus `engine.v1.hosts.projection_status`, an explicit handler-adapter registry, and `select_available_builtins` degrading on missing required vs. optional capabilities — composed into `KernelComposition` at real `build_engine_server` startup via `bootstrap.AdditionalEngineFeatures`, raising a typed `EngineCompositionError` on operation collision, incompatible kernel API, a missing required capability, or a dependency cycle, and a verified minimal vendor-free composition serving the 5 core operations over a real socket with zero `model_deck.integrations.*`/`model_deck.adapters.providers.*` modules loaded; new coverage in `test_builtin_descriptors.py`, `test_kernel_startup.py`, `test_minimal_composition.py`, `test_kernel_composition.py`, and a new architecture negative fixture (`builtin_private_engine.py`) in `test_architecture_check.py`; full gate green (development-guard 37, engine 1629, 0 failures). A second B17 delivery 2026-09-15 (commit `1100448`) closed the routing gap this row previously flagged: a new `BuiltinDispatchBinding` (`engine/builtins/dispatch_binding.py`) holds one handler per built-in operation over the unchanged private dispatch method that already served it, `dispatch.py`'s `handle()` if-ladder is gone, and every built-in operation — including the pre-existing ones — now reaches its registered descriptor handler through `KernelComposition.invoke_dispatch_bound`; `jobs.get`/`jobs.cancel` did **not** execute through the kernel before this commit, despite this row's prior wording, and now do. `dispatch_routed_handlers`/`DispatchRoutedOperationError` no longer exist. New coverage in `test_builtin_dispatch_binding.py`. | Two items remain, both stated in `docs/kernel.md`: the bound handler bodies are still private `EngineDispatch` methods that answer with whole JSON-RPC envelopes (`KernelPassthroughError`) rather than plain results, so a built-in cannot yet be composed into a kernel with no dispatch behind it; and `ExtensionGateway` (`engine/extensions/ports.py`) declares `job_get`/`job_cancel` but not `job_resume`, so "a directory may serve some job methods and not others" is enforced only by `dispatch.py`'s own skip rule, not by the protocol implementers read. | None for boundary repair |
| B18 | Supervised external plugin/provider protocol | **Implementation remaining** | Process runtime/invocation channels, [provider proxy](../../../python/src/model_deck/plugins/provider_proxy/README.md), archived deterministic-provider integration; provider proxy and external-provider integration re-conformed to the provider-neutral message contract on 2026-09-15 (G6). Heartbeat, crash-backoff/restart-loop, and unexpected-worker-death supervision delivered 2026-09-15 (commits `67d4ace`, `2824797`): `ProcessRuntime` reports worker loss and tells a dead child from a heartbeat-missing-but-alive one; `ProcessExtensionActivationLifecycle` settles an unexpected death exactly like an operator quiesce (jobs interrupted, identity revoked, activation unpublished, runtime closed) and a new bounded `ActivationRestartSupervisor` replaces the activation through the existing validate->admit path with backoff, giving up visibly; startup sweeps every prior activation's jobs so a reopened engine never shows a plugin job running from a dead process. Explicit resume delivered the same day (commits `e38ddcc`, `161614b`): `PluginJobBroker.checkpoint`, `SQLitePluginJobRepository` checkpoint/resume-receipt storage, `ResumeJobUseCase`, and a host-side resume transport (`_ResumeInvoker`), proven end to end in [`test_plugin_recovery_workflow.py`](../../../python/tests/engine/test_plugin_recovery_workflow.py) against the new [`examples/job-recovery-fixture/`](../../../examples/job-recovery-fixture/README.md) packaged plugin: start, crash mid-job, detect the loss, settle the interrupted job, restart under backoff, explicit resume to completion, a hung worker caught by heartbeat, restart exhaustion, and a clean shutdown/reopen that preserves every job's terminal state. | Finish resource/slow-reader/owned-descendant guarantees (not exercised by the new recovery suite); prove credential scope and model-library selection for a supervised/restarted worker; and build a shared built-in/external provider conformance suite. `test_plugin_recovery_workflow.py`'s own docstring names two further gaps: `extensions.get`'s frozen result schema has no room for worker health/restart-attempts/gave_up, so recovery evidence is read from `host.supervision_report()` rather than a public operation; and no shutdown report carries a child pid, because `ProcessRuntime` exposes none. | B17 composition and B19 brokers |
| B19 | Brokered plugin data, jobs, and events | **Implementation remaining** | Serving external plugins receive storage/job brokers; public `jobs.get`/`jobs.cancel`, owner/activation persistence, bounded result retrieval, terminal exclusivity, cancellation acknowledgement, worker-loss interruption, and restart non-replay pass through the real socket/SQLite path. Safe explicit resume delivered 2026-09-15 (commits `794bf34`, `e38ddcc`, `161614b`): the frozen `plugin.v1.broker.jobs.checkpoint`/`engine.v1.jobs.resume` contracts, a checkpoint-compare-and-swap broker method, `begin_resume`/`rollback_resume` storage transactions, and `ResumeJobUseCase`'s not_found/capability_denied/idempotent-replay/conflict/resume_unavailable/plugin_unavailable ladder, proven against real subprocess plugins in `test_job_resume.py` and `test_plugin_recovery_workflow.py`. | Add event-subscription revocation and confused-deputy/content-grant coverage. The delivered resume path intentionally does not replay interrupted work and resumes exactly once per idempotency key under serial use; a plugin invocation that fails after the durable resume transition commits is rolled back to INTERRUPTED so a fresh key can retry, but two resumes racing on the same key in parallel (rather than serially, as a client does) can both reach the invoker before the transition commits — documented as a known limit, not fixed. | B18 serving supervisor |
...
| B25 | Native presenters and platform attachment parity | **Integration remaining** | Extracted model/usage/settings/platform components, [V2 app](../../../macos/Sources/ModelDeckV2/README.md), relocated native model/Notebook/export/restart/disable acceptance, qualified D1a Desktop attachment, and native fresh-state OpenRouter/model/Codex preparation with Keychain-backed credentials and restart-safe status | Finish the Codex settings editor; prove all five states, generation/main-actor/geometry/focus/appearance parity and the remaining lifecycle cases; remove legacy orchestration only afterward. The visible Desktop picker remains outside the protected-session GUI run. | B10, B15, B16, B21 |
| B26 | Immutable packaging, compatibility, docs, aggregate G7 | **Integration remaining** | [legacy-compatible stager](../../../scripts/package/README.md), [V2 builder](../../../scripts/v2/README.md), [catalog](../../architecture/CATALOG.md), and one relocated checkout-independent V2 app with a retained external-runtime contract | Finish the engine/SDK artifacts and aggregate inventory; prove actual signatures/helper identity, no bytecode mutation, V2 schema-version/upgrade/recovery evidence, completed settings editor, behavior-matrix mapping, and implemented G7 and an all-local run that includes it (G0-G6 are implemented and were green on 2026-09-15). V2 remains unsigned and depends on an external Python environment. | B06, B15-B16, B23, B25 |
| B27 | Fresh-install qualification and authorized tool switch | **Blocked** | V2 coding, Notebook, and [D1a Desktop attachment](CODEX-DESKTOP-QUALIFICATION-2026-09-14.md) are isolated milestone evidence | Finish B26, then separately authorize exact target/state paths and qualify installation, streaming/cancel/compaction/restart/plugin update/helper identity and recovery. | B26 plus separate operational authorization |
...
- Visible Codex Desktop UI interaction, parallel tool calls, live-provider opaque compaction,
