...
| B14 | Cursor execution adapter | **Implementation remaining** | [Cursor provider guide](../../../docs/providers/cursor.md), coordinator/process-runtime/profile tests, V2 profile composition, and isolated live Composer 2.5 proof for tool editing, passing checks, follow-up context, interruption, usage, and exact owned-process cleanup | Move retained broker installation/update mechanics wholly behind the package and add one package-owned fake-SDK process fixture covering account routing, reasoning/Fast refusal, truncated terminals, and feature-loading isolation. Live proof complements but does not replace the original no-network gate. | Retained compatibility broker; B15 for provider-native continuation |
