...
| G4 | `python3 scripts/verify.py migration --state-root <s> --artifact-root <a>` | the explicit module manifest `MIGRATION_EVIDENCE_MODULES` in `scripts/verify.py`, which names the modules proving each G4 evidence item below and is printed per evidence item and again as the deduplicated selected list so it can be audited; a named module that no longer exists fails the gate, the host-projection rendering tests (`test_projection_policy`, `test_conditional_projection_files`, `test_model_projection_invalidation`, `test_projection_dependency_expansions`) are deliberately excluded because they prove rendering rather than migration or recovery, and overlap with G2 is intentional |
