...
- **Delivered 2026-09-15:** Commits `09336e4` and `1198ab3` landed `engine.builtins` — a capability vocabulary, 12 `FeatureDescriptor`s covering the full `_OPERATION_CATALOG` plus `engine.v1.hosts.projection_status`, an explicit handler-adapter registry, and `select_available_builtins` degrading on missing required vs. optional capabilities — composed into `KernelComposition` at real `build_engine_server` startup through `bootstrap.AdditionalEngineFeatures`. Startup now raises a typed `EngineCompositionError` on operation collision, incompatible kernel API, a missing required capability, or a dependency cycle, and a minimal vendor-free composition is verified serving the 5 core operations over a real socket with zero `model_deck.integrations.*`/`model_deck.adapters.providers.*` modules loaded. New tests: `test_builtin_descriptors.py`, `test_kernel_startup.py`, `test_minimal_composition.py`, `test_kernel_composition.py`, and a new `builtin_private_engine.py` architecture negative fixture. What remained at that point: built-in operation ROUTING was still static, not descriptor-driven — `dispatch.py`'s `handle()` if-chain executed every pre-existing built-in operation directly, and the descriptor-bound handlers for those operations (`dispatch_routed_handlers`) were `DispatchRoutedOperationError` placeholders that existed only to make the composition complete. Only the operations C8 added in this same wave (prices/benchmarks/usage.summary query+refresh) actually executed through `KernelComposition.invoke` — `jobs.get`/`jobs.cancel` did **not** execute through the kernel yet at that point, despite being cited elsewhere as if they did.
- **Delivered 2026-09-15 (closure, commit `1100448`):** Closed the routing gap above. A new `BuiltinDispatchBinding` (`engine/builtins/dispatch_binding.py`) holds one handler per built-in operation over the unchanged private `EngineDispatch` method that already served it — the same error codes, messages, projection triggers, and event publication, now reached through `KernelComposition.invoke_dispatch_bound` instead of the if-ladder, which is gone. Every built-in operation, `jobs.get`/`jobs.cancel` included, now actually executes through the kernel route; `dispatch_routed_handlers`/`DispatchRoutedOperationError` are deleted. New coverage: `test_builtin_dispatch_binding.py`. A same-day repair (folded into the same commit on this branch) fixed a job-directory fan-out bug — a directory unable to serve a method used to abort the whole search instead of letting another directory try — and removed the caller-facing message parameter from `KernelDomainError`, so a composed feature can choose the domain error *code* but never its public text (the engine now renders one fixed sentence per code). What remains: the bound handler bodies are still private `EngineDispatch` methods that answer with whole JSON-RPC envelopes (`KernelPassthroughError`) rather than plain results, so a built-in still cannot be composed into a kernel with no dispatch behind it; and `ExtensionGateway` (`engine/extensions/ports.py`) declares `job_get`/`job_cancel` but not `job_resume`, so "a directory may serve some job methods and not others" is enforced only by dispatch's own skip rule, not by the protocol implementers read.
...
- **Delivered 2026-09-15:** Commits `67d4ace` and `2824797` landed heartbeat, crash-backoff/restart-loop, and dependency-failure (unexpected worker death) supervision. `ProcessRuntime` gained an optional `worker_loss_listener`, a daemon heartbeat thread that beats `plugin.v1.heartbeat` and skips a beat whenever an exchange is already in flight, and a `health()`/`heartbeat_supported` surface distinguishing a dead child from a heartbeat-missing-but-alive one. `ProcessExtensionActivationLifecycle` settles an unexpected worker death exactly like an operator quiesce (jobs interrupted, identity revoked, activation unpublished, runtime closed) behind one per-activation latch fed by both the listener and an independent liveness poll, then hands off to a new `ActivationRestartSupervisor` that replaces the activation through the existing validate->admit path with bounded backoff and gives up visibly; startup sweeps every prior activation's jobs so a reopened engine never shows a plugin job running from a dead process. New tests: `test_worker_heartbeat.py`, `test_worker_health_records.py`, `test_worker_loss_supervision.py`, `test_worker_restart_backoff.py`; end-to-end proof against real subprocess plugins in `test_plugin_recovery_workflow.py` (C3/C8's e2e module, described there). What remains: resource/slow-reader/owned-descendant guarantees are not exercised by any of the new tests; the installed provider's credential scope and model-library selection are not proven under supervision/restart; and no shared built-in/external provider conformance suite exists. `extensions.get`'s frozen result schema has no room for worker health/restart-attempts/gave_up, so that evidence is read through `host.supervision_report()` instead of a public operation, and no shutdown report carries a child pid because `ProcessRuntime` exposes none.
...
- **Delivered 2026-09-15:** Commit `794bf34` froze `plugin.v1.broker.jobs.checkpoint` and `engine.v1.jobs.resume` (params/result), an optional `resumable`/`resume_count` on `jobs.get.result`, and an optional `resumable` on a manifest operation contribution. Commits `e38ddcc` and `161614b` implemented safe explicit resume on top: `PluginJobBroker.checkpoint` re-authorizes through the existing trusted-followup path and enforces a compare-and-swap on `expected_revision`; `SQLitePluginJobRepository` gained `resumable`/`resume_count` columns (migrated, old rows default non-resumable), a resume-receipt table, and `read_checkpoint`/`begin_resume`/`rollback_resume`/`read_resume_receipt`; `ResumeJobUseCase` implements not_found/capability_denied/idempotent-replay/conflict/resume_unavailable/plugin_unavailable, invoking the plugin only after the durable transition commits and rolling back to INTERRUPTED if that invocation then fails. `engine.v1.jobs.resume` is a built-in descriptor operation reached through C1's dispatch binding. Proven against a real subprocess plugin in `test_job_resume.py` (11 cases including the `resumed_from_revision: null` no-checkpoint-yet path) and end to end in `test_plugin_recovery_workflow.py` against `examples/job-recovery-fixture/`. New coverage: `test_plugin_job_broker.py`, `test_plugin_job_wire.py`, `test_sqlite_plugin_jobs.py`, `test_jobs_use_cases.py`, `test_job_resume.py`. What remains: event-subscription revocation and confused-deputy/content-grant coverage are not addressed by this delivery. The delivered path intentionally does not replay interrupted work and resumes exactly once per idempotency key under serial (client) use; two resumes racing on the same key in parallel can both reach the invoker before the durable transition commits — a known, documented limit, not fixed.
...
- **Delivered 2026-09-15:** Commits `1383d35`, `8f953d9`, `c39b222`, `b5e3001`, and `70328ce` landed the full slice. Frozen `price_record`/`benchmark_record`/`source_provenance`/`cost_kind` vocabulary plus `prices.query`/`prices.refresh`/`benchmarks.refresh`/`usage.summary` schemas; OpenRouter price and Artificial-Analysis/Design-Arena benchmark source adapters with reverse-domain provenance; an engine evidence cache (`engine/evidence`) with age-bearing provenance, atomic refresh-or-keep-last-good behavior on fetch failure, and settled/estimated/subscription `cost_kind` projection (`project_cost_kind`, guarded so an estimate can never carry settled money); reserved first-party jobs under the `com.modeldeck.engine.` plugin_id prefix backing `engine.v1.prices.refresh`/`engine.v1.benchmarks.refresh` — confirming the Dependencies note above: no public job-create operation was added, only first-party-owned jobs observable through the existing `jobs.get`/`jobs.cancel`; and MCP evidence reads with legacy fallback plus a V2 price-snapshot row and a legacy-ledger cost-kind projection in the Swift usage view. Full gate green: development-guard 37, contracts, engine 1629, swift 243, migration 321, providers 335, extensions 362 tests, 0 failures across all. What remains: no subscription-allowance source is composed (`SubscriptionAllowancePort` has no adapter, so `subscription_allowance` never appears in the engine's `usage.summary`, though the legacy Swift dashboard does derive a subscription label from recorded Codex plan-meter fields as a separate, presentation-level fallback); benchmark `provider_model_id` is never mapped from a feed's `source_model_ref` to a registered model; only one benchmark feed is composed per refresh with no cross-feed merge/rank; and `EvidenceResourceExhaustedError` still maps to a flat `internal` rather than `resource_exhausted` in dispatch. Provider monetary cost remains unknown by design.
...
2026-09-15: MiniMax-M3 and GPT-5.6 Sol agent routes were unavailable from the orchestrating session; implementation ran on Claude sonnet/opus subagents, reported per AGENTS.md.
